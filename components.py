...
def display_select_mode():
    ...
    回答モードのラジオボタンを表示
    """
    # 回答モードをサイドバーに表示する（見出しは h2 にしたまま、下の余白を詰める）
    st.sidebar.markdown("<h2 style='margin:0; padding:0;'>利用目的</h2>", unsafe_allow_html=True)
...
def display_app_greeting():
    """
    タイトル下に表示する挨拶文（メイン領域）
    """
    # チャットバブル（assistant）として挨拶文を表示
    with st.chat_message('assistant'):
        # 添付画像の色に合わせて背景と文字色を指定
        st.markdown(
            "<div style='background:#e6f7e6; color:#2e7d32; padding:12px; border-radius:8px;'>こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。サイドバーで利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。</div>",
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        # 参照元ドキュメントのページ番号が取得できた場合にのみ、PDFにページ番号を表示
                        main_path = message['content']['main_file_path']
                        if "main_page_number" in message["content"] and main_path.lower().endswith('.pdf'):
                            # 以前に保存されていたページ番号が 0 始まりの場合に備え、表示は 1 始まりに揃える
                            try:
                                display_main_page = int(message['content']['main_page_number'])
                            except Exception:
                                display_main_page = message['content']['main_page_number']
                            st.success(f"{main_path} （ページNo.{display_main_page}）", icon=icon)
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                sub_source = sub_choice['source']
                                if "page_number" in sub_choice and sub_source.lower().endswith('.pdf'):
                                    try:
                                        display_sub_page = int(sub_choice['page_number'])
                                    except Exception:
                                        display_sub_page = sub_choice['page_number']
                                    st.info(f"{sub_source} （ページNo.{display_sub_page}）", icon=icon)
                                else:
                                    st.info(sub_source, icon=icon)
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            # ページ番号を取得（initialize 側で 1 始まりに正規化されている想定）
            main_page_number = llm_response["context"][0].metadata["page"]
            # 「メインドキュメントのファイルパス」と「ページ番号」を表示（PDF のみページ表示）
...
def display_contact_llm_response(llm_response):
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            # ページ番号が取得できた場合のみ、PDFファイルではページ番号を表示
            if "page" in document.metadata:
                # ページ番号を取得
